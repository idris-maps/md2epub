...
interface TreeFile { fileName: string, xhtml: string }
interface TreeFolder { label: string, xhtml: string, children: (TreeFile | TreeFolder)[] }
...
const buildTree = (
  ...
  const children = Object.keys(d)
    .map(key => buildTree(d[key], sourceFolder, destinationFolder, [...path, key]))
...
export const folderIsFile = (d: FolderTree): d is TreeFile => !Object.keys(d).includes('children')
