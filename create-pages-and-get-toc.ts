...
import { createFolderTree, folderIsFile, FolderTree } from './create-folder-tree.ts'
...
import { last, init, tail } from './utils.ts'
import { createFolderRecursive } from './create-folder-recursive.ts'

const sequential = async <A,B>(func: (d: A, i: number, prev?: B) => Promise<B>, arr: A[], i = 0, res: B[] = [], prev?: B): Promise<B[]> => {
...
const getLabel = (firstLine: string, fileName: string) => {
  ...
  if (fromFirstLine !== '') {
    return fromFirstLine.length > 41 ? fromFirstLine.slice(0, 40) + '' : fromFirstLine
  }
...
const createXhtml = async (part: FolderTree, count: number): Promise<FileTree> => {
  ...
  return {
    ...
    playOrder: count + 1,
    children: await sequential((d, i) => createXhtml(d, i + 1 + count), part.children)
  }
}

const createAllXhtml = (parts: FolderTree[]) =>
  sequential((d: FolderTree, _i: number, prev?: FileTree): Promise<FileTree> => {
...
export const createPagesAndGetToc = async (sourceFolder: string, destinationFolder: string) =>
  createAllXhtml(
    createFolderTree(
