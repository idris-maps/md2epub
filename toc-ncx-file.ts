...
import { getFileId, getRelativePath } from './utils.ts'

const INDENT = '  '

const renderNav = ({ label, xhtml, children, playOrder }: FileTree, indent = INDENT): string=> {
  const childs = children ? children.map(d => renderNav(d, indent + INDENT)) : []
  return [
